...
{% include figure.html image_path="/images/image-inpainting/occluded.png"
   alt="Occluded CIFAR-10 images" dims="1580x371" width="90"
   caption="Figure 1. Examples of CIFAR-10 images with a randomly placed 10×10 occlusion mask." %}
...
{% include figure.html image_path="/images/image-inpainting/architecture.png"
   alt="CAE architecture diagram" dims="1016x302" width="90"
   caption="Figure 2. Baseline CAE architecture: encoder (left) compresses to latent space; decoder (right) reconstructs." %}
...
{% include figure.html image_path="/images/image-inpainting/baseline_loss.png"
   alt="Baseline training loss curve" dims="1200x600" width="80"
   caption="Figure 3. Training and validation loss curves for the baseline model (Adam, latent=128, 10×10 mask)." %}
...
{% include figure.html image_path="/images/image-inpainting/baseline_results.png"
   alt="Baseline predicted vs ground truth" dims="3000x900" width="90"
   caption="Figure 4. Predicted reconstructions vs. ground-truth images for the baseline model." %}
...
{% include figure.html image_path="/images/image-inpainting/optimizer_loss_curves.png"
   alt="Optimizer loss curves" dims="2100x750" width="90"
   caption="Figure 5. Training and validation loss curves for Adam, RMSprop, and SGD+Momentum." %}

{% include figure.html image_path="/images/image-inpainting/optimizer_visual.png"
   alt="Optimizer visual comparison" dims="1024x506" width="90"
   caption="Figure 6. Predicted vs. ground-truth reconstructions for each optimizer." %}
...
{% include figure.html image_path="/images/image-inpainting/latent_loss_curves.png"
   alt="Latent dimension loss curves" dims="2100x750" width="85"
   caption="Figure 7. Training and validation loss for latent channel counts of 16, 64, 128, and 256." %}

{% include figure.html image_path="/images/image-inpainting/latent_visual.png"
   alt="Latent dimension visual comparison" dims="1026x605" width="90"
   caption="Figure 8. Predicted vs. ground-truth reconstructions for each latent dimension." %}
...
{% include figure.html image_path="/images/image-inpainting/mask_loss_curves.png"
   alt="Mask strategy loss curves" dims="2100x750" width="90"
   caption="Figure 9. Training and validation loss for different masking strategies." %}

{% include figure.html image_path="/images/image-inpainting/mask_visual.png"
   alt="Mask strategy visual comparison" dims="3000x2310" width="90"
   caption="Figure 10. Predicted vs. ground-truth reconstructions for each masking strategy." %}
...
{% include figure.html image_path="/images/image-inpainting/baseline_multi_square_mse.png"
   alt="Baseline MSE vs number of masks" dims="1200x600" width="85"
   caption="Figure 11. Baseline model test MSE as the number of 10×10 test-time masks increases from 1 to N." %}

{% include figure.html image_path="/images/image-inpainting/baseline_multi_square_heatmap.png"
   alt="Per-image heatmap" dims="1950x600" width="85"
   caption="Figure 12. Per-image MSE heatmap across all 10 test images as mask count increases. Each row is one image; each column is a mask count." %}
...
{% include figure.html image_path="/images/image-inpainting/150_11_mask.png"
   alt="Example of 150 masks on a cat" dims="1121x544" width="40"
   caption="Figure 13. Example input: 150 masks of size 11×11 applied to a 224×224 image (~36% occluded)." %}

---

## 4. Model Architecture

{% include figure.html image_path="/images/image-inpainting/vit_architecture.png"
   alt="ViT architecture" dims="721x237" width="90"
   caption="Figure 14. Vision Transformer architecture used for inpainting." %}
...
{% include figure.html image_path="/images/image-inpainting/early_perceptual.png"
   alt="Early perceptual loss failure" dims="1122x540" width="60"
   caption="Figure 15. Output when perceptual loss is introduced too early ($\lambda_{\text{MAE}}$=0.7, $\lambda_{\text{Perceptual}}$=0.3 at epoch 20). The model produces RGB noise that persists throughout training." %}

By epoch 20, outputs are blurry but structurally recognisable.

{% include figure.html image_path="/images/image-inpainting/1_MAE_0_perp.png"
   alt="Epoch 20 reconstruction" dims="1121x541" width="60"
   caption="Figure 16. Reconstruction at epoch 20 ($\lambda_{\text{MAE}}$=1.0, $\lambda_{\text{Perceptual}}$=0.0). Blurry but structurally correct." %}
...
{% include figure.html image_path="/images/image-inpainting/exploding_gradient.png"
   alt="Gradient explosion" dims="1713x462" width="90"
   caption="Figure 17. Gradient explosion caused by an abrupt weight shift. Left: loss spike in the training plot. Right: output degenerates to a uniform grey image." %}
...
{% include figure.html image_path="/images/image-inpainting/epoch_40_results.png"
   alt="Epoch 40 reconstruction" dims="1628x550" width="90"
   caption="Figure 18. Reconstruction at epoch 40. The transition was handled without gradient explosion. Residual RGB noise is visible but manageable." %}
...
{% include figure.html image_path="/images/image-inpainting/epoch_135.png"
   alt="Epoch 135 reconstruction" dims="1702x539" width="80"
   caption="Figure 19. Reconstruction at epoch 135. Structure is well-recovered; patch-boundary mosaic artefact is evident." %}
...
{% include figure.html image_path="/images/image-inpainting/epoch_200_results.png"
   alt="Epoch 200 reconstruction" dims="1685x529" width="80"
   caption="Figure 20. Reconstruction at epoch 200. Mosaic artefacts are significantly reduced; edge continuity is improved." %}
...
{% include figure.html image_path="/images/image-inpainting/epoch_270_results.png"
   alt="Epoch 270 reconstruction" dims="1786x572" width="80"
   caption="Figure 21. Reconstruction at epoch 270 (150 masks, size 11×11, ~36% occlusion). Quality is well-maintained despite increased difficulty." %}
...
{% include figure.html image_path="/images/image-inpainting/epoch_350_results.png"
   alt="Epoch 350 reconstruction" dims="1702x543" width="80"
   caption="Figure 22. Reconstruction at epoch 350 (200 masks, size 11×11, ~48% occlusion)." %}
...
{% include figure.html image_path="/images/image-inpainting/epoch_400_results.png"
   alt="Epoch 400 reconstruction" dims="1620x515" width="80"
   caption="Figure 23. Reconstruction at epoch 400 (200 masks, size 12×12). High occlusion is handled with reasonable fidelity." %}
...
{% include figure.html image_path="/images/image-inpainting/training_progress_plot.png"
   alt="Full training loss plot" dims="1687x860" width="80"
   caption="Figure 24. Training and test loss across all 400 epochs. Phase transitions are visible as loss spikes, each followed by a return to a lower baseline than before." %}
...
{% include figure.html image_path="/images/image-inpainting/generalize.png"
   alt="Generalisation test" dims="1756x978" width="95"
   caption="Figure 25. Left: 150 masks. Centre: 200 masks. Right: 250 masks. Test images are drawn from outside the training set. Reconstructions remain coherent across increasing occlusion levels." %}
...
{% include figure.html image_path="/images/image-inpainting/ships_and_boats.png"
   alt="Ships and boats generalisation" dims="1741x297" width="95"
   caption="Figure 26. Inpainting results on ships and boats — categories unseen during training. Overall shape and colour distribution are preserved, though reconstructions are blurrier than for animal images." %}
